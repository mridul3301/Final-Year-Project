...
namespace Aircraft
{
	...
	{	
		...
		[Tooltip("If true,enable traning mode")]
		...
		///<summary>
		/// Finds aircraft agentsa in the area
		///</summary>
		...
		public void ResetAgentPosition(AircraftAgent agent, bool randomize = false)
		{
			...
			//Set start position to the previoius checkpoint
			...
			//Get the oriemtation at that position on the race path 
			Quaternion oriemtation = racePath.EvaluateOrientation(startPosition);

			// Calculate a horizontal offset so the agents are spread out
			Vector3 positionOffset = Vector3.right * (AircraftAgents.IndexOf(agent) - AircraftAgents.Count / 2f)
				* UnityEngine.Random.Range(9f, 10f);
				...
				agent.transform.position = basePosition + oriemtation * positionOffset;
				agent.transform.rotation = oriemtation;
		}
	}
}
